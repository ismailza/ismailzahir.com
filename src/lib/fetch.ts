...
export async function getAllServices(): Promise<ServiceProps[]> {
  const filePath = path.join(process.cwd(), 'src', 'app', 'data', 'services.json');
  const jsonData = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(jsonData);
}
 ...
 */
export async function getAllHonors(): Promise<HonorProps[]> {
  const filePath = path.join(process.cwd(), 'src', 'app', 'data', 'honors.json');
  const jsonData = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(jsonData);
}